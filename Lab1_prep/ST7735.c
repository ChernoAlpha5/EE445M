...
// 2 displays, 4 lines per display
void ST7735_Message (int device, int line, char *string, int32_t value){
	ST7735_FillRect(0, 78, 128, 2, ST7735_RED);
	if (line >= 0 && line < 8){
		int strLen; int lineNum;
		if (device == 0){
			lineNum = line;
		}
		else{
		...
		strLen = ST7735_DrawString(0, lineNum, string, ST7735_YELLOW);
		ST7735_SetCursor(strLen + 1, lineNum);  //set cursor to end of string
		if (value < 0){ //if negative number draw "-" before number
			value *= -1;
			ST7735_DrawString(strLen + 1, lineNum, "-", ST7735_YELLOW);
			ST7735_SetCursor(strLen + 2, lineNum);
		}
