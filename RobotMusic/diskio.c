...
#define SDC_CS           (*((volatile uint32_t *)0x40004004))
#define SDC_CS_LOW       0           // CS controlled by software
#define SDC_CS_HIGH      0x01
void CS_Init(void){
  SYSCTL_RCGCGPIO_R |= 0x01;            // activate clock for Port D
  ...
  GPIO_PORTA_DIR_R |= 0x01;             // make PD7 out
  GPIO_PORTA_AFSEL_R &= ~0x01;          // disable alt funct on PD7
  GPIO_PORTA_DR4R_R |= 0x01;            // 4mA drive on outputs
  GPIO_PORTA_PUR_R |= 0x01;             // enable weak pullup on PD7
  GPIO_PORTA_DEN_R |= 0x01;             // enable digital I/O on PD7
                                        // configure PD7 as GPIO
  GPIO_PORTA_PCTL_R = (GPIO_PORTD_PCTL_R&0xFFFFFFF0)+0x00000000;
  GPIO_PORTA_AMSEL_R &= ~0x01;          // disable analog functionality on PD7
  SDC_CS = SDC_CS_HIGH;
}
//********SSI0_Init*****************
