...
int OS_AddPeriodicThread(void(*task)(void),unsigned long period, unsigned long priority){
	long status = StartCritical();
	if (WideTimer0A_Available()){//Wide Timer0A
		WTimer0A_Init(task, period, priority);
	}
	else if(WideTimer0B_Available()){	//Wide Timer0B
...
void OS_RemovePeriodicThread(void(*task)(void)){
	if(task == WideTimer0A_Task()){
		WideTimer0A_Stop();
	}
	else if(task == WideTimer0B_Task()){
		WideTimer0B_Stop();
	}
}
