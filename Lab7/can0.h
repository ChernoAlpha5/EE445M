...
#define RCV_ID 2 // make conditional compilation flag
#define XMT_ID 4
...
int CAN0_GetMailNonBlock(uint8_t data[MESSAGE_LENGTH], uint8_t messageNumber);

// if receive data is ready, gets the data 
// if no receive data is ready, it waits until it is ready
void CAN0_GetMail(uint8_t data[MESSAGE_LENGTH], uint8_t messageNumber);
