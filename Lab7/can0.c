...
uint8_t static Messages[NUM_MESSAGES][MESSAGE_LENGTH];
int static MailFlag[NUM_MESSAGES];
      ...
      if( (0x1 << i) & ulIDStatus){  // if active, get data
        ...
        if(xTempMsgObject.ulMsgID == RCV_ID){
					...
					for(j=0; j<MESSAGE_LENGTH; j+=1){
						Messages[messageNumber][j] = data[j];
					}
          MailFlag[messageNumber] = true;   // new mail
        }
      }
    }
  }
...
int CAN0_CheckMail(uint8_t messageNumber){
  return MailFlag[messageNumber];
}
// if receive data is ready, gets the data and returns true
// if no receive data is ready, returns false
int CAN0_GetMailNonBlock(uint8_t data[MESSAGE_LENGTH], uint8_t messageNumber){ int i;
  if(MailFlag[messageNumber]){
		for(i = 0 ; i<MESSAGE_LENGTH; i+=1){
      data[i] = Messages[messageNumber][i];
		}
    MailFlag[messageNumber] = false;
    return true;
  }
  return false;
}
...
void CAN0_GetMail(uint8_t data[MESSAGE_LENGTH], uint8_t messageNumber){ int i;
